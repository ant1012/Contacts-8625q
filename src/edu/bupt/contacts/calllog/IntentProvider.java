...
public abstract class IntentProvider {
    ...
    // public static IntentProvider getPlayVoicemailIntentProvider(final long
    // rowId,
    ...
    public static IntentProvider getCallDetailIntentProvider(final CallLogAdapter adapter, final int position,
            final long id, final int groupSize) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {
                mcontext = context;
                ...
                // String voicemailUri =
                // cursor.getString(CallLogQuery.VOICEMAIL_URI);
                // if (voicemailUri != null) {
    ...
    public static IntentProvider getCallSearchDetailIntentProvider(final long id) {
        return new IntentProvider() {
            @Override
            public Intent getIntent(Context context) {

                Log.v("IntentProvider", "context: " + context);
                // Intent intent = new Intent(CallLogSearchAdapter.mContext,
                // CallDetailActivity.class);
                Intent intent = new Intent(context, CallDetailActivity.class);

                intent.putExtra(CallDetailActivity.EXTRA_VOICEMAIL_START_PLAYBACK, false);

                // use the direct URI for it.
                intent.setData(ContentUris.withAppendedId(Calls.CONTENT_URI, id));
